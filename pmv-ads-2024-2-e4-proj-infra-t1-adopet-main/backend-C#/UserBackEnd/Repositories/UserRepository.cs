...
namespace UserBackend.Repositories
{
    public class UserRepository : IUserRepository
    {
        ...
        public async Task<UpdateUserResponse> UpdateUser(UpdateUserRequest newUser)
        {
            ...
            try
            {
                ...
                UpdateDefinition<UserDbo> update = Builders<UserDbo>.Update
                    .Set(userDb => userDb.Name, newUser.Name)
        ...
        {
            FilterDefinition<UserDbo> filter = Builders<UserDbo>.Filter.Eq(u => u.Id, userId);
            var userDeleted = await _Users.DeleteOneAsync(filter);
            return userDeleted.IsAcknowledged;
        }
    }
}
