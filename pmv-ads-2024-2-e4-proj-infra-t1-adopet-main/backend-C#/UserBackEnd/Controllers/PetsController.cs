...
namespace UserBackend.Controllers
{
    ...
    public class PetsController : ControllerBase
    {
        ...
        [HttpGet("filter")]
        public async Task<IActionResult> GetPetsByFilter(int Age, string City = "All", string State = "All", ESpecie Specie = ESpecie.All)
        {
            try
            {
                List<PetDto> filteredPets = await _petService.GetPetsByFilter(Age, City, State, Specie);
        ...
        public async Task<IActionResult> DeletePet(string id)
        {
            try
            {
                ...
                return Ok("Pet succefully deleted");
            }
            catch (Exception ex)
            {
                return BadRequest(ex.Message);
